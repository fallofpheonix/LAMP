...
|---|---:|---|
| Task 1: Path Tracing | **92** | `outputs/predicted_paths.gpkg`, `outputs/lost_path_candidates.geojson`, `outputs/probability_heatmap.tif`, `outputs/run_summary.json`, `outputs/prior_training_report.json` |
...
### Scoring Basis
- Task 1 scoring rubric:
1. CV prior training artifacts present: 20/20
2. Probabilistic route inference between terminals: 20/20
3. Lost-path recovery outputs: 20/20
4. GIS vector exports (paths + centerlines): 20/20
5. Validation robustness: 12/20 (`run_summary f1=0.3033`, `topk_recall=0.5614`, limited terminal count)
